...
- **B.** (Deprecated) Migrar a Mapbox Standard Style con `showLandmarkIconLabels` si aplica (verificar compatibilidad con react-map-gl).
...
- [x] Landmarks visibles vía showLandmarkIcons/showLandmarkIconLabels (FLOWYA).
...
- **OPEN_LOOPS / bitácora:** Referencia a este plan al cerrar micro-scopes.
