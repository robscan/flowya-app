...
| `explore-country-filter-chip-row.tsx` | Sí | Sí (**ds-run-explore-filter-chips**) | Modal filtros lugares, `CountriesSheet` (detalle país) |
| `explore-filter-chips-showcase.tsx` | Sí | Sí (**ds-run-explore-filter-chips**) | — (demo chips etiqueta OR + país) |
| `explore-tag-filter-chip-row.tsx` | Sí | Sí (**ds-run-explore-filter-chips** + **ds-run-surface**) | `SearchSurface`, modal filtros, mapa |
...
| `explore-welcome-sheet.tsx` | Sí | Sí (**ds-tpl-explore-welcome**: un escenario alto + una sheet; toggle lista vacía) | Cold-start Explorar; snap compartido con CountriesSheet: bitácora `332`. |
...
|---------------------|-----------|---------|------|
| `components/design-system/explore-welcome-sheet.tsx` + `explore-chrome-shell.tsx` | Shell producto | Vitrina **`ds-tpl-explore-welcome`** + `ds-pat-explore` | `ExploreChromeShell` unifica banda KPI + sheet bienvenida; `ExploreWelcomeSheet` lista cold-start. Re-export deprecado en `components/explorar/ExploreWelcomeSheet.tsx`. Bitácoras `331`, `332`. |
...
[^1]: Última actualización de inventario vinculada a cierre de producto: bitácoras `330`, `332` (abril 2026).
