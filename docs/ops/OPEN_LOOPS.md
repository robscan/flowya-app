...
**Sincronización 2026-04-26:** bloque auxiliar sin abrir OL nuevo para cerrar hallazgos de revisión y endurecer estabilidad de build/deploy: share preview público en `/s/:id` con HTML server-side, anti-duplicado exacto (sin bloqueo por substring) y `lib/supabase.ts` con lazy init para que `expo export` no falle por import eager del cliente. Evidencia: bitácora [`378`](../bitacora/2026/04/378-share-preview-anti-duplicado-build-hardening.md). **Loop activo sin cambio:** `OL-CONTENT-001`.
...
- **Gate 2026-04-20 resuelto antes de continuar ejecución:** bloque correctivo P0 documentado en bitácoras [`373`](../bitacora/2026/04/373-apertura-operativa-saneamiento-docs-bugs-previos-loop.md) y [`375`](../bitacora/2026/04/375-qa-cerrado-aclaracion-legal-fotos-publicas.md). Tras el QA, se ejecuta el bloque auxiliar [`376`](../bitacora/2026/04/376-etiquetas-v2-multipais-profile-panel-bulk.md) sin abrir un loop ejecutivo nuevo; **mergeado en `main` en PR #158**; `OL-CONTENT-001` sigue siendo el loop activo único.
...
- **En espera (cola — sin incluir el loop activo):**
  1. **OL-GLOBAL-SHELL-SEARCH-001** — Primer slice del Plan Maestro V1: shell mobile-first con bottom nav `Explore/Flow/Passport`, avatar superior izquierdo para `Account`, Search global superior derecho, feature flag/ruta interna para V1 Next y criterios iOS/Android antes de crecer dominios. Fuente: [`FLOWYA_V1_MASTER_PLAN.md`](../product-definition/FLOWYA_V1_MASTER_PLAN.md).
