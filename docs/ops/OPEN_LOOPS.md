...
- **En espera (cola — sin incluir el loop activo):**
  ...
  4. **OL-EXPLORE-FILTERS-MODAL-SIDEBAR-001** — Hints etiquetas/países, edición etiquetas, variante sidebar desktop. *Follow-up copy (entrada Lugares):* alinear microcopy del CTA sheet (`ExplorePlacesActiveFiltersBar`, hoy «Etiquetas y filtros») con título / secciones del modal (`ExplorePlacesFiltersModal`, hoy «Filtros») — ver bitácora [`364`](../bitacora/2026/04/364-ol-explore-filters-entry-layout-sheet-search-row.md) § deuda.
